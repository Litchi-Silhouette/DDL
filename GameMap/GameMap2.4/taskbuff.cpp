...
void TaskBuff::twinkle(){
    if(activated && not isVisible()) show();
    else hide();
}

void TaskBuff::set_twinkle(){
    qDebug()<<"set twinkle";
    connect(ptwinkle_timer,&QTimer::timeout,this,&TaskBuff::twinkle);
    ptwinkle_timer->start(200);
}

...
double GameMapThree::GRAVITY = 4.0;
...
void GameMapThree::move_event(){
    ...
    if(x_player < 0){
        ...
        qDebug()<<"right reflect"<<x_player<<" "<<pfigure->width()<<" "<<GameMapThree::X_RIGHT_BARRIER;
        qDebug()<<pwarningline->geometry()<<" "<<MainWindow::CELL_SIZE;
    }
    if(y_player < 0){
...
void GameMapThree::time_event_bullet_hit_player(){
    ...
    {
        if(pblt->activated &&
            calculate_distance(pblt,pfigure) < 0.5 * pfigure->size().width())
        {
